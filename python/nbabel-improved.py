...
RE: Improvements that can be made:
    ...
    1. Done (but need to explore more) - Many of the places the author has used v**2 for squaring, instead we can use any NumPy functions like np.square(), np.pow(), np.multiply()
    2. Done - Some places np.sum is used but in some places sum() is used, it should be generalized
    3. Oops! I was wrong - Author should use if-else condition while passing sys.argv[1]  
    4. Done - Conversion of python2 to python3
    5. Why to load data in main function? Class should be created for loading data! It should be generalized.

TODOs:
    
    1. Compare different functions of numpy, which takes less time use that
    2. How to improve my coding skills?
    3. Use Numba, Cython in next
"""

"""
Note:
    1. Comparision (np.sum() | sum()) :- https://stackoverflow.com/questions/10922231/pythons-sum-vs-numpys-numpy-sum
"""
...
# Done - TODO: Name of class capital
class Particle(object):
    ...
    def kinetic_energy(self):
        return np.multiply(0.5, np.multiply(self.mass, np.sum(np.power(v,2) for v in self.velocity)))
        # return np.prod(0.5, self.mass, np.sum(np.fromiter((np.power(v,2) for v in self.velocity), float)))
...
class Cluster(list):
    ...
    @property
    def kinetic_energy(self):
        return np.sum(particle.kinetic_energy for particle in self)
    ...
    def __accelerate(self):
        ...
        for p1, p2 in combinations(self, 2):
            ...
            distance = np.sqrt(np.sum(np.power(vector,2)))
            p1.acceleration[0] = np.subtract(p1.acceleration[0], np.multiply((np.divide(p2.mass,np.power(distance, 3))), vector))
            p2.acceleration[0] = np.add(p2.acceleration[0], np.multiply((np.divide(p1.mass, np.power(distance, 3))), vector))
            self.potential_energy = np.subtract(self.potential_energy, np.divide((np.multiply(p1.mass, p2.mass)), distance))
    ...
    def __advance_positions(self, dt):
        for p in self:
            ...
            p.position = np.add(p.position, np.add(np.multiply(p.velocity, dt), np.multiply(0.5, np.multiply(np.power(dt,2), p.acceleration[0]))))
    ...
    def __advance_velocities(self, dt):
        for p in self:
            ...
            p.velocity = np.add(p.velocity, np.multiply(0.5, np.multiply(np.add(p.acceleration[0], p.acceleration[1]), dt)))
